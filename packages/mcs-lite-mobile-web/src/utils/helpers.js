...
export function assertSourcesSinks(sources, sinks, main, done, timeOpts = {}) {
  ...
  const _sources = Object.keys(sources).reduce(
    (_sources, sourceKey) => {
  ...
  const _sinks = Object.keys(sinks).reduce(
    (_sinks, sinkKey) => {
      const sinkObj = sinks[sinkKey];
      const diagram = Object.keys(sinkObj)[0];
      const sinkOpts = sinkObj[diagram];

      return Object.assign(_sinks, {
        [sinkKey]: Time.diagram(diagram, sinkOpts),
      });
    },
    {}
  );
